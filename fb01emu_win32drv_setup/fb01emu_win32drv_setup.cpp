...
const char FB01EMU_DRIVER_NAME[] = "fb01emu.dll";
...
const char SUCCESSFULLY_INSTALLED_MSG[] = "FB01Emu MIDI Driver successfully installed";
const char SUCCESSFULLY_UPDATED_MSG[] = "FB01Emu MIDI Driver successfully updated";
const char SUCCESSFULLY_UNINSTALLED_MSG[] = "FB01Emu MIDI Driver successfully uninstalled";
...
const char CANNOT_INSTALL_NO_PORTS_ERR[] = "Cannot install FB01Emu MIDI driver:\n There are no MIDI ports available";
const char CANNOT_REGISTER_ERR[] = "Cannot register driver";
const char CANNOT_UNINSTALL_ERR[] = "Cannot uninstall FB01Emu MIDI driver";
const char CANNOT_UNINSTALL_NOT_FOUND_ERR[] = "Cannot uninstall FB01Emu MIDI driver:\n There is no driver registry entry found";
const char CANNOT_INSTALL_PATH_TOO_LONG_ERR[] = "FB01Emu MIDI Driver cannot be installed:\n Installation path is too long";
const char CANNOT_INSTALL_FILE_COPY_ERR[] = "FB01Emu MIDI Driver failed to install:\n File copying error";
...
bool registerDriver(bool &installMode) {
	...
	for (int i = 0; i < 10; i++) {
		...
		if (!_stricmp(str, FB01EMU_DRIVER_NAME)) {
			RegCloseKey(hReg);
			installMode = false;
			return true;
		}
	...
	res = RegSetValueExA(hReg, drvName, NULL, REG_SZ, (LPBYTE)FB01EMU_DRIVER_NAME, sizeof(FB01EMU_DRIVER_NAME));
	if (res != ERROR_SUCCESS) {
...
void constructDriverPathName(char *pathName) {
	...
	strncat(pathName, FB01EMU_DRIVER_NAME, MAX_PATH - strlen(pathName));
}
...
void deleteFileReliably(char *pathName) {
	...
	char tmpFilePrefix[sizeof(FB01EMU_DRIVER_NAME) + 1];
	strncpy(tmpFilePrefix, FB01EMU_DRIVER_NAME, sizeof(FB01EMU_DRIVER_NAME));
...
int main(int argc, char *argv[]) {
	...
	if (pathDelimPosition != NULL && setupPathLen > MAX_PATH - sizeof(FB01EMU_DRIVER_NAME) - 2) {
	...
	strncat(setupPathName, FB01EMU_DRIVER_NAME, MAX_PATH - strlen(setupPathName));
