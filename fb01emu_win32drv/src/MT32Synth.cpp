...
static MidiSynth &midiSynth = MT32Synth::getInstance();
...
MidiSynth &MT32Synth::getInstance() {
	static MT32Synth instance;
	return instance;
}

// Renders all the available space in the single looped ring buffer
void MT32Synth::RenderAvailableSpace() {
...
void MT32Synth::Render(Bit16s *bufpos, DWORD framesToRender) {
	synthEvent.Wait();
...
unsigned int MT32Synth::MillisToFrames(unsigned int millis) {
	return UINT(sampleRate * millis / 1000.0f);
}

void MT32Synth::LoadWaveOutSettings() {
	HKEY hReg;
...
Bit32u MT32Synth::getMIDIEventTimestamp() {
...
void MT32Synth::PlayMIDI(DWORD msg) {
	synth->playMsg(msg, getMIDIEventTimestamp());
}

void MT32Synth::PlaySysex(const Bit8u *bufpos, DWORD len) {
	synth->playSysex(bufpos, len, getMIDIEventTimestamp());
}

void MT32Synth::FreeROMImages() {
	if (controlROM != NULL) {
